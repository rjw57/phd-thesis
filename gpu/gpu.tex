...
\begin{figure}[p]
\centering
\includegraphics[height=0.37\textheight]{screw_cube_before} \\
\noindent (a) \\ \rule{0pt}{\parskip} \\
\includegraphics[height=0.37\textheight]{screw_cube_after} \\
\noindent (b) 
...
Sicne the internals of GPUs are not publically available it is only possible to speculate
as to the reason of this but it might be related to vertex cache size. If all of the 
model vertices fiti within the on-GPU vertex cache they may be processed efficiently without
aacessing graphics memory. If the number of vertices exceed the vertex cache size then
