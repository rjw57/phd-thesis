\chapter{Interpolation via Logarithms}
...
\begin{figure}[t]\centering
\includegraphics[width=0.6\columnwidth]{interp}
...
We firstly resolve a three dimensional orthonormal basis, $\{a,b,t_\perp\}$,
relative to $P$
%as shown in figure 
%\ref{fig:plane_basis}. Here
...
It can be shown \cite{jic23fyr} that when setting the dilation component,
...
% We now consider the path of a simple linear interpolation which includes a dilation term. 
% Now consider the additional dilation component. Full derivation is given in \cite{jic23fyr}
% and here we simply present the result. Again we represent the position of the interpolant
% at $\lambda$ as $r_\lambda$
% \begin{eqnarray}
%   r_\lambda &=&-\frac1\omega|t_\Jperp|\hat{t}_\Jperp - \frac{\omega}{\omega^2 + \phi^2}|t_\Jpar|\hat{t}_\Jpar - \frac{\phi}{\phi^2 + \omega^2}|t_\Jpar|\hat{t}_\Jparperp\nonumber\\
%   &&+ e^{-2\lambda\omega}\left(\frac{|t_\Jperp|}\omega \hat{t}_\Jperp + \frac{\omega\cos(2\lambda\phi) - \phi\sin(2\lambda\phi)}{\omega^2 + \phi^2}|t_\Jpar|\hat{t}_\Jpar + \frac{\omega\sin(2\lambda\phi) + \phi\cos(2\lambda\phi)}{\omega^2 + \phi^2}|t_\Jpar|\hat{t}_\Jparperp\right)\nonumber\\
% \end{eqnarray}
% where $\hat{t}_\Jparperp$ is a unit vector perpendicular to both $t_\perp$ and
% $t_\parallel$.
% The terms in the first line specify a constant offset.  Naturally this goes to $\frac{|t_\Jpar|}2\hat{t}_\Jparperp$ in the case where there is no dilation term in the bivector.  The second term is the sum of a pair of Conical Helices and can be shown to be a single conical helix giving the path in the alternative form.
% \begin{eqnarray}
% r_\lambda &=&-\frac1\omega|t_\Jperp|\hat{t}_\Jperp - \frac{\omega}{\omega^2 + \phi^2}|t_\Jpar|\hat{t}_\Jpar - \frac{\phi}{\phi^2 + \omega^2}|t_\Jpar|\hat{t}_\Jparperp\nonumber\\
%   &&+ e^{-2\lambda\omega}\left(\frac{|t_\Jperp|}\omega \hat{t}_\Jperp + \frac{\cos\left(2\lambda\phi + \tan^{-1}\left(\frac{\phi}{\omega}\right)\right)}{\sqrt{\omega^2 + \phi^2}}|t_\Jpar|\hat{t}_\Jpar+ \frac{\sin\left(2\lambda\phi + \tan^{-1}\left(\frac\phi\omega\right)\right)}{\sqrt{\omega^2 + \phi^2}}|t_\Jpar|\hat{t}_\Jparperp\right)\nonumber\\
% \end{eqnarray}
