...
It is worth comparing this method of rotation to the method involving
...
\]
where $q_0^2 + q_1^2 + q_2^2 + q_3^2 = 1$. Interpolation between rotations represented
by $\{ q_1, \cdots, q_i \}$ is then performed by interpolating each of $q_i$
over the surface of a four-dimensional hyper-sphere. If we are interpolating
between unit quaternions $q_0$ and $q_1$ the SLERP interpolation is
\[
q = \left\{
\begin{array}{ll}
